...
def enable_token():
    ...
    token=subprocess.check_output("kubectl get secrets -o jsonpath=\"{.items[?(@.metadata.annotations['kubernetes\.io/service-account\.name']=='eks-admin')].data.token}\"|base64 --decode", shell=True).decode("utf-8")
